...
import { TrendingUp, Target, Settings, Sun, Moon, Coffee, LogOut, Sparkles } from 'lucide-react';
...
const Header = ({ onOpenSettings }) => {
  ...
  useEffect(() => {
    ...
    const loadMoodScore = async () => {
      ...
      try {
        ...
        console.log('📊 Humeurs chargées:', moods); // Debug
        ...
        if (moods.length > 0) {
          ...
          console.log('🎯 Humeur dominante:', maxMood); // Debug
          
          // Calculer son pourcentage
          const percentage = (maxMood[1] / moods.length) * 100;
          setMoodScore(Math.round(percentage));
          setDominantMood(maxMood[0]); // Stocker l'état de l'humeur dominante
        } else {
          setMoodScore(0);
  ...
  const getScoreDisplay = () => {
    ...
    // Couleur basée sur l'humeur dominante
    switch(dominantMood) {
  ...
  const IconComponent = scoreDisplay.icon;
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="p-3 bg-white/60 rounded-xl hover:bg-white/80 transition-all shadow-lg hover:scale-105">
